 ...
 *
 * @purpose
 * - Displays the profile page of a student, including personal details, enrolled courses,
 *   achievements, and relevant statistics.
 * - Provides a tabbed interface to switch between personal info, courses, and achievements.
 *
 * @param None
 * @returns {JSX.Element} React component rendering the student profile page with:
 *   - Personal details (name, photo, DOB, email, phone, address, school, grade, branch)
 *   - List of enrolled courses with progress and attendance
 *   - Student achievements and milestones
 *   - Breadcrumb navigation for page context
 * @throws None
 * @sideEffects
 * - Uses `useAuth` hook to fetch authenticated user information.
 * - Manages local component state using `useState` (active tab selection).
 *
 * @example
 * <StudentProfile />
 * // Renders the student profile page for the logged-in user, showing personal info,
 * // courses, and achievements in separate tabs.
 */
export default function StudentProfile() {
   ...
   *
   * @purpose
   * - Provides personal information of the student for display in the profile page.
   *
   * @param None
   * @returns {Object} Student object containing:
   *   - `id` {string} Unique student ID.
   *   - `name` {string} Full name of the student.
   *   - `photo` {string} URL of the student's avatar or photo.
   ...
   *   - `joinDate` {string} Enrollment date.
   *   - `branch` {string} Branch of the Institutiontute where student is enrolled.
   * @throws None
   * @sideEffects None; pure data object
   *
   * @example
   * console.log(student.name); // "Yatheen Kumar"
   ...
   *
   * @purpose
   * - Provides a structured list of courses with progress and schedule information.
   *
   * @param None
   ...
   *   - `level` {string} Difficulty level of the course.
   *   - `progress` {number} Progress percentage of the student in the course.
   *   - `startDate` {string} Start date of the course.
   *   - `nextClass` {string} Next scheduled class timing.
   *   - `attendance` {string} Attendance percentage in the course.
   * @throws None
   * @sideEffects None; pure data object
   *
   * @example
   * console.log(courses[0].name); // "Guitar Lessons"
   * console.log(courses[1].attendance); // "88%"
   */
  const courses = [
    {
      id: 1,
   ...
   *
   * @purpose
   * - Displays milestones, certifications, or accomplishments earned by the student.
   *
   * @param None
   * @returns {Array<Object>} Array of achievement objects:
   *   - `id` {number} Unique achievement ID.
   *   - `title` {string} Title of the achievement.
   *   - `type` {string} Type of achievement (e.g., "Performance", "Certification", "Milestone").
   *   - `date` {string} Date when the achievement was earned.
   *   - `description` {string} Short description of the achievement.
   * @throws None
   * @sideEffects None; pure data object
   *
   * @example
   * console.log(achievements[0].title); // "First Recital"
   * console.log(achievements[2].type); // "Milestone"
   */
  const achievements = [
    {
      id: 1,
      ...
      description:
        "Successfully performed 'Scarborough Fair' at the school concert",
    },
    {
      id: 2,
      ...
      description:
        "Passed the Music Theory Level 1 examination with distinction",
    },
    {
      id: 3,
      ...
      description:
        "Consistently practiced for at least 30 minutes daily for 3 months",
    },
  ];

  /**
   * Breadcrumb links for the Student Profile page.
   *
  ...
  return (
    ...
    <AppShell>
      ...
      {/* Grid layout for student information and courses */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          {/* Card showing Personal Information and Courses */}
          <Card>
            <Tabs
