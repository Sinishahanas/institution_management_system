...
/**
 * BranchAdminTeachers — page component for managing teacher employees.
 *
 * @purpose Render teacher list, provide create/edit/view flows and perform
 *   CRUD-like updates for teacher records.
 * @param None
 * @returns {JSX.Element} React component for the Branch Admin Teachers page.
 * @throws None
...
export default function BranchAdminTeachers() {
   ...
   *
   * @purpose
   * Retrieves the complete employee list from `/api/employees` and exposes it to the component.
   *
   * @params
   * None
   *
   ...
   *
   * @throws
   * Will throw an error if the API call fails internally within React Query.
   *
   * @sideEffects
   ...
   *
   * @purpose
   * Extracts only employees with `position === "teacher"` and sorts them in descending order
   * based on their numeric Employee ID (removing the "TCR" prefix).
   *
   * @params
   * employees {Employee[]} - The list of employees fetched via useQuery.
   *
   * @returns
   * {Employee[]} - Sorted list of teacher employees by Employee ID (highest first).
   *
   * @throws
   * None
   *
   * @sideEffects
   * None (pure function with memoization).
   *
   * @example
   * ```tsx
   ...
   *
   * @purpose
   * Retrieves all users, including those linked to teachers, from `/api/users`.
   *
   * @params
   * None
   *
   ...
   *
   * @throws
   * Throws an internal error if the `/api/users` request fails.
   *
   * @sideEffects
   ...
   *
   * @purpose
   * Retrieves the complete list of courses from `/api/courses`
   * to be used for teacher specialization options.
   *
   * @params
   * None
   *
   * @returns
   * {Object} - Returns:
   *   - data {Course[]} The list of courses (default empty array).
   *
   * @throws
   * Will throw internally if the `/api/courses` request fails.
   *
   * @sideEffects
   ...
   *
   * @purpose
   * Retrieves the full list of branches from `/api/branches`
   * to associate teachers with one or more branches.
   *
   * @params
   * None
   *
   * @returns
   * {Object} - Returns:
   *   - data {Branch[]} The list of branches (default empty array).
   *
   * @throws
   * Will throw internally if the `/api/branches` request fails.
   *
   * @sideEffects
  ...
  const columns: ColumnDef<Employee>[] = [
    ...
    },
    {
       ...
       * @throws None
       * @sideEffects Uses `formatSalaryWithoutSymbol` helper function.
       * @example
       * `5,000.00`
       */
      cell: ({ row }) => {
   ...
   *
   * @purpose Accepts an input that may be a string, an array of strings, or objects
   *   (sometimes coming from DB representations like `"{1,2}"`) and returns a flat
   *   array of string IDs suitable for mapping and display.
   *
   * @params
   * - items: any — Input representing branch ids. May be a string (e.g. `"{1,2}"`),
   *   a comma-separated string like `"1,2"`, an array of such values, or other types.
   *
   ...
   *
   * @sideEffects
   * - Logs the intermediate parsing steps to console for debugging.
   *
   * @example
   ...
   *
   * @purpose Normalize `branch` property on each teacher (ensures it's an array of strings)
   *   and filter teachers based on current `activeTab`.
   *
   ...
   *
   * @purpose Creates both a user account (in `/api/users`) and an employee record
   *   (in `/api/employees`) in a single flow. If user creation succeeds but employee
   *   creation fails, attempts to roll back the created user.
   *
   * @param None directly (mutationFn receives `data` when mutation is triggered)
   *
   * @mutationFn Params
   * - data: any — Payload containing the teacher and user fields (employeeId, firstName, lastName, userName, password, email, branch, salary, etc).
   *
